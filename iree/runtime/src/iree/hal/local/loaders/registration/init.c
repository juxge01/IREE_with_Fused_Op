...
#if defined(IREE_HAVE_HAL_EXECUTABLE_LOADER_ACCEL_MODULE)
#include "iree/hal/local/loaders/accel_module_loader.h"
#endif  // IREE_HAVE_HAL_EXECUTABLE_LOADER_ACCEL_MODULE
...
#if defined(IREE_HAVE_HAL_EXECUTABLE_LOADER_ACCEL_MODULE)
  if (iree_status_is_ok(status)) {
    status = iree_hal_accel_module_loader_create_isolated(
        /*user_module_count=*/0, /*user_modules=*/NULL, host_allocator,
        &loaders[count++]);
  }
#endif  // IREE_HAVE_HAL_EXECUTABLE_LOADER_ACCEL_MODULE
...
#if defined(IREE_HAVE_HAL_EXECUTABLE_LOADER_ACCEL_MODULE)
  if (iree_string_view_equal(name, IREE_SV("accel-module"))) {
    return iree_hal_accel_module_loader_create_isolated(
        /*user_module_count=*/0, /*user_modules=*/NULL, host_allocator,
        out_executable_loader);
  }
#endif  // IREE_HAVE_HAL_EXECUTABLE_LOADER_ACCEL_MODULE
