...
#include "iree/hal/local/loaders/accel_module_loader.h"
...
//===----------------------------------------------------------------------===//
// iree_hal_accel_worker_state_t
//===----------------------------------------------------------------------===//

typedef struct iree_hal_accel_worker_state_t {
  // Context containing both the ACCEL module and the loaded executable.
  // This context may also contain custom user modules available for the
  // generated ACCEL modules to use.
  iree_vm_context_t* context;

  // Pointer into the ACCEL module state for the worker context.
  // This is used to update module state directly.
  iree_vm_module_state_t* accel_module_state;
} iree_hal_accel_worker_state_t;
 
static iree_status_t iree_hal_accel_worker_state_initialize(
    ...
    iree_allocator_t host_allocator, iree_hal_accel_worker_state_t* out_state) {
  ...
  // Fetch the ACCEL module state so that we can quickly access it to set
  // per-call state.
  iree_vm_module_t* accel_module = modules[IREE_ACCEL_MODULE_INDEX];
  iree_vm_module_state_t* accel_module_state = NULL;
  iree_status_t status = iree_vm_context_resolve_module_state(
      context, accel_module, &accel_module_state);
  ...
  if (iree_status_is_ok(status)) {
    ...
    out_state->accel_module_state = accel_module_state;
  } else {
...
static void iree_hal_accel_worker_state_deinitialize(
    iree_hal_accel_worker_state_t* state) {
...
static const iree_hal_local_executable_vtable_t iree_hal_accel_executable_vtable;
...
static iree_status_t iree_hal_accel_executable_verify_entry_point(
  ...
  if (!iree_string_view_equal(
          ...
          iree_make_cstring_view(IREE_ACCEL_ENTRY_SIGNATURE))) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "executable entry point does not match the expected calling "
        "convention; expected '" IREE_ACCEL_ENTRY_SIGNATURE
...
static iree_status_t iree_hal_accel_executable_create(
    iree_vm_instance_t* instance, iree_host_size_t module_count,
    iree_vm_module_t** modules, iree_vm_module_t* bytecode_module,
  ...
  *out_executable = NULL;
  ...
  if (iree_status_is_ok(status)) {
    ...
    iree_hal_local_executable_initialize(&iree_hal_accel_executable_vtable,
                                         host_allocator, &executable->base);
    executable->base.dispatch_attrs = dispatch_attrs;

    executable->worker_capacity = worker_capacity;
    executable->worker_states = (iree_hal_accel_worker_state_t*)ptr;
  ...
  if (iree_status_is_ok(status)) {
    for (iree_host_size_t i = 0; i < worker_capacity; ++i) {
      status = iree_hal_accel_worker_state_initialize(
...
static void iree_hal_accel_executable_destroy(
  ...
  for (iree_host_size_t i = 0; i < executable->worker_capacity; ++i) {
    iree_hal_accel_worker_state_deinitialize(&executable->worker_states[i]);
  }
...
static iree_status_t iree_hal_accel_executable_issue_call(
  ...
  iree_hal_accel_worker_state_t* worker_state =
      &executable->worker_states[worker_id];
  iree_accel_module_state_update_workgroup_state(
      worker_state->accel_module_state, workgroup_state->processor_id);
  ...
  // Map bindings into on-stack ACCEL buffers.
  ...
  //
  //   func.func @entry(
  //       %local_memory: !accel.buffer,
  //       %constants: !accel.buffer,
  //       %bindings: !util.list<!accel.buffer>,
  //       %workgroup_id_x: i32,
  //       %workgroup_id_y: i32,
  //       %workgroup_id_z: i32,
...
//===----------------------------------------------------------------------===//
// iree_hal_accel_module_loader_t
//===----------------------------------------------------------------------===//

typedef struct iree_hal_accel_module_loader_t {
...
static const iree_hal_executable_loader_vtable_t
    iree_hal_accel_module_loader_vtable;

iree_status_t iree_hal_accel_module_loader_create(
  ...
  *out_executable_loader = NULL;
  ...
  // A single ACCEL module is shared across all loaded executables.
  iree_vm_module_t* accel_module = NULL;
  IREE_RETURN_IF_ERROR(
      iree_accel_module_create(instance, host_allocator, &accel_module));

  iree_host_size_t common_module_count = 1 + user_module_count;
  iree_hal_accel_module_loader_t* executable_loader = NULL;
  ...
  if (iree_status_is_ok(status)) {
    ...
    // We prepend the accel_module to any user-provided modules.
    ...
    executable_loader->common_modules[IREE_ACCEL_MODULE_INDEX] = accel_module;
    iree_vm_module_retain(accel_module);
  ...
  iree_vm_module_release(accel_module);
  IREE_TRACE_ZONE_END(z0);
  return status;
}

iree_status_t iree_hal_accel_module_loader_create_isolated(
  ...
  iree_status_t status = iree_hal_accel_module_loader_create(
...
static void iree_hal_accel_module_loader_destroy(
    iree_hal_executable_loader_t* base_executable_loader) {
  iree_hal_accel_module_loader_t* executable_loader =
      (iree_hal_accel_module_loader_t*)base_executable_loader;
...
static bool iree_hal_accel_module_loader_query_support(
    iree_hal_executable_loader_t* base_executable_loader,
...
static iree_status_t iree_hal_accel_module_loader_try_load(
    iree_hal_executable_loader_t* base_executable_loader,
    const iree_hal_executable_params_t* executable_params,
  ...
  if (iree_status_is_ok(status)) {
    ...
    status = iree_hal_accel_executable_create(
...
static const iree_hal_executable_loader_vtable_t
    iree_hal_accel_module_loader_vtable = {
        .destroy = iree_hal_accel_module_loader_destroy,
        .query_support = iree_hal_accel_module_loader_query_support,
        .try_load = iree_hal_accel_module_loader_try_load,
};
