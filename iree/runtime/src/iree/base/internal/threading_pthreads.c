...
iree_status_t iree_thread_create(iree_thread_entry_t entry, void* entry_arg,
  ...
  // }
  // >>> PATCH: guard tiny/invalid stack sizes to avoid EINVAL from pthread_create
  if (params.stack_size) {
    size_t req = params.stack_size;

    // 안전 최소 스택 (보수적으로 256KB; 플랫폼에서 더 크면 그 값을 사용)
    size_t min_stack = 256 * 1024;
#if defined(PTHREAD_STACK_MIN)
    if (min_stack < PTHREAD_STACK_MIN) min_stack = PTHREAD_STACK_MIN;
#endif

    if (req < min_stack) {
      // 너무 작게 요청되면 설정을 생략해 OS 기본 스택을 사용
      req = 0;
    } else {
      // 페이지 크기 정렬(일부 플랫폼에서 요구)
      long pagesz = sysconf(_SC_PAGESIZE);
      if (pagesz > 0) {
        size_t pz = (size_t)pagesz;
        if (req % pz) req = ((req + pz - 1) / pz) * pz;
      }
    }

    if (req) {
      // 실패(EINVAL 등)는 무시하고 OS 기본으로 진행
      (void)pthread_attr_setstacksize(&thread_attr, req);
    }
  }

  // >>> PATCH: 스케줄링은 부모 상속(명시적 정책/우선순위로 인한 EINVAL 방지)
  (void)pthread_attr_setinheritsched(&thread_attr, PTHREAD_INHERIT_SCHED);
  ...
  *out_thread = thread;
  ...
  int rc;
  {
    ...
    if (rc != 0) {
      // fprintf(stderr, "pthread_create failed rc=%d (stack_size=%zu)\n", rc, params.stack_size);
      fprintf(stderr,
              "pthread_create failed rc=%d (requested_stack=%zu)\n",
              rc, params.stack_size);
      // affinity 설정 코드가 있다면 그것도 rc 출력
      return iree_make_status(IREE_STATUS_INTERNAL, "thread creation failed with %d", rc);
    }
  }
