...
void transferOffloadAttr(IREE::Util::FuncOp srcFuncOp, Operation *destOp) {
  Attribute attr = srcFuncOp->getAttr("offload.target");
  if (attr) {
    destOp->setAttr("offload.target", attr);
  }
}
...
public:
  ...
  void runOnOperation() override {
    ...
    for (auto &info : convertedFuncInfos) {
      ...
      // Propagate offload.target attribute to dispatch regions.
      info.funcOp.walk(
          [&](IREE::Flow::DispatchOp dispatchOp) {
            transferOffloadAttr(info.funcOp, dispatchOp);
          });
    }
  }
  ...
  LogicalResult convertFuncOp(func::FuncOp torchFunc,
    ...
    if (auto offloadAttr = torchFunc->getAttr("offload.target")) {
      asyncFuncOp->setAttr("offload.target", offloadAttr);
    }
