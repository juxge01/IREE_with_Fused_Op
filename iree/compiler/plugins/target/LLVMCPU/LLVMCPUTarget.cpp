...
//  public:
//   // Constructor now accepts LLVMTargetOptions, just like LLVMCPUTargetBackend
//   explicit AccelTargetBackend(const LLVMTargetOptions &options)
//       : options_(options) {}
...
//   void getDefaultExecutableTargets(
//       MLIRContext *context, StringRef deviceID, DictionaryAttr deviceConfigAttr,
//       SmallVectorImpl<ExecutableTargetAttr> &executableTargetAttrs) const override {
//     Builder b(context);
//     SmallVector<NamedAttribute> configItems;
//     // Use the stored options to configure the target
//     options_.target.storeToConfigAttrs(context, configItems);
//     auto config = b.getDictionaryAttr(configItems);
//     executableTargetAttrs.push_back(
//         ExecutableTargetAttr::get(context, b.getStringAttr("accel"),
//                                   b.getStringAttr("accel-elf-riscv"), config));
//   }

//   void getDependentDialects(DialectRegistry &registry) const override {
//     // For now, let's keep this minimal
//     registry.insert<IREE::Codegen::IREECodegenDialect>();
//   }

//   void buildTranslationPassPipeline(ExecutableTargetAttr targetAttr,
//                                     OpPassManager &passManager) override {
//     // TODO: Add your custom pipeline
//     buildLLVMCPUCodegenPassPipeline(passManager);
//   }

//  private:
//   const LLVMTargetOptions &options_;
// };
...
struct LLVMCPUSession
  ...
  void populateHALTargetBackends(IREE::HAL::TargetBackendList &targets) {
    // 1. 기존 llvm-cpu 등록 (그대로 둠)
    targets.add("llvm-cpu", [this]() {
      return std::make_shared<LLVMCPUTargetBackend>(options.getTargetOptions());
    });

    // 2. 테스트를 위해, 'accel'이라는 이름으로 **LLVMCPU 백엔드**를 한번 더 등록
    // targets.add("accel", [this]() {
    //   return std::make_shared<AccelTargetBackend>(options.getTargetOptions());
    // });
  }
};
