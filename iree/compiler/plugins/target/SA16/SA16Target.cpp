...
#include "compiler/plugins/target/SA16/Builtins/Device.h"
#include "compiler/plugins/target/SA16/Builtins/Musl.h"
#include "compiler/plugins/target/SA16/Builtins/UKernel.h"
#include "compiler/plugins/target/SA16/SA16IRPasses.h"
#include "compiler/plugins/target/SA16/SA16TargetOptions.h"
#include "compiler/plugins/target/SA16/SA16LibraryBuilder.h"
#include "compiler/plugins/target/SA16/SA16LinkerTool.h"
#include "compiler/plugins/target/SA16/SA16StaticLibraryGenerator.h"
...
#define DEBUG_TYPE "iree-sa16-target"
using llvm::dbgs;
...
class SA16TargetBackend final : public TargetBackend {
public:
  explicit SA16TargetBackend(SA16TargetOptions options)
  ...
  void getDefaultExecutableTargets(
      ...
      SmallVectorImpl<ExecutableTargetAttr> &executableTargetAttrs)
      const override {
  ...
  void getHostExecutableTargets(MLIRContext *context, StringRef deviceID,
    ...
    std::optional<SA16Target> maybeTarget = SA16Target::createForHost();
    if (maybeTarget) {
      executableTargetAttrs.push_back(
  ...
  ExecutableTargetAttr getExecutableTarget(MLIRContext *context, const SA16Target &target) const {
    ...
    return b.getAttr<ExecutableTargetAttr>(
        b.getStringAttr("sa16"), b.getStringAttr(format),
  ...
  void
  buildConfigurationPassPipeline(ExecutableTargetAttr targetAttr,
                                 OpPassManager &passManager) override {
    buildACCELCodegenConfigurationPassPipeline(passManager);
  }
  ...
  void buildTranslationPassPipeline(ExecutableTargetAttr targetAttr,
    ...
    buildACCELCodegenPassPipeline(passManager, enableAArch64SME);
  }

  void buildLinkingPassPipeline(OpPassManager &passManager) override {
    buildACCELLinkingPassPipeline(passManager, "sa16");
  }
  ...
  getVariantTarget(IREE::HAL::ExecutableVariantOp variantOp) {
    ...
    return SA16Target::loadFromConfigAttr(variantOp.getLoc(), configAttr,
  ...
  LogicalResult serializeExecutable(const SerializationOptions &options,
    ...
    if (!maybeTarget)
      return failure();
    const SA16Target &target = *maybeTarget;
    LLVM_DEBUG(dbgs() << "SA16 SerializeExecutable:\n"
                      << "-----------------------------\n";
               target.print(dbgs()));
    ...
    SA16LibraryBuilder::Mode libraryBuilderMode =
        target.debugSymbols ? SA16LibraryBuilder::Mode::INCLUDE_REFLECTION_ATTRS
                            : SA16LibraryBuilder::Mode::NONE;
    SA16LibraryBuilder libraryBuilder(llvmModule.get(), libraryBuilderMode,
                                  SA16LibraryBuilder::Version::LATEST);

    switch (target.sanitizerKind) {
    case SanitizerKind::kNone: {
      libraryBuilder.setSanitizerKind(SA16LibraryBuilder::SanitizerKind::NONE);
      break;
    }
    case SanitizerKind::kAddress: {
      libraryBuilder.setSanitizerKind(SA16LibraryBuilder::SanitizerKind::ADDRESS);
      for (auto &function : llvmModule->getFunctionList()) {
        function.addFnAttr(llvm::Attribute::SanitizeAddress);
      }
    } break;
    case SanitizerKind::kThread: {
      libraryBuilder.setSanitizerKind(SA16LibraryBuilder::SanitizerKind::THREAD);
      for (auto &function : llvmModule->getFunctionList()) {
    ...
    for (auto exportOp : variantOp.getBlock().getOps<ExecutableExportOp>()) {
      ...
      SA16LibraryBuilder::DispatchAttrs dispatchAttrs = {0};
      ...
      SA16LibraryBuilder::SourceLocation sourceLocation;
      if (options.debugLevel >= 1) {
        if (auto loc = findFirstFileLoc(exportOp.getLoc())) {
          sourceLocation = {"", loc->getFilename().str(), loc->getLine()};
        }
      }
      SmallVector<SA16LibraryBuilder::SourceLocation> stageLocations;
    ...
    std::unique_ptr<SA16LinkerTool> linkerTool;
    if (!target.linkStatic) {
      ...
      SA16TargetOptions options = defaultOptions_;
      options.target = target;
      linkerTool = SA16LinkerTool::getForTarget(targetTriple, options);
      if (!linkerTool) {
        return mlir::emitError(variantOp.getLoc())
    ...
    if (failed(
            SA16::runLLVMIRPasses(target, targetMachine.get(), llvmModule.get()))) {
      return variantOp.emitError()
    ...
    // This must come first such that we have the proper library linking order.
    {
      ...
      if (failed(SA16::runEmitObjFilePasses(targetMachine.get(), llvmModule.get(),
  ...
  LogicalResult serializeStaticLibraryExecutable(
      const SerializationOptions &options, const SA16Target &target,
  ...
  LogicalResult serializeDynamicLibraryExecutable(
      ...
      const SmallVector<Artifact> &objectFiles, SA16LinkerTool *linkerTool) {
    ...
    if (target.getLinkEmbedded()) {
      ...
      auto bufferAttr = DenseIntElementsAttr::get(
          VectorType::get(ArrayRef<int64_t>{static_cast<int64_t>(elfFile->size())},
      ...
      auto bufferAttr = DenseIntElementsAttr::get(
          VectorType::get(ArrayRef<int64_t>{static_cast<int64_t>(libraryFile.size())},
                          IntegerType::get(executableBuilder.getContext(), 8)),
...
private:
  ...
  // settings.
  const SA16TargetOptions defaultOptions_;
};

struct SA16Session
    : public PluginSession<SA16Session, SA16TargetCLOptions,
                           PluginActivationPolicy::DefaultActivated> {
  void populateHALTargetBackends(IREE::HAL::TargetBackendList &targets) {
    targets.add("sa16", [this]() {
      return std::make_shared<SA16TargetBackend>(options.getTargetOptions());
    });
  }
};
...
IREE_DEFINE_COMPILER_OPTION_FLAGS(
    mlir::iree_compiler::IREE::HAL::SA16TargetCLOptions);

extern "C" bool iree_register_compiler_plugin_hal_target_sa16(
    mlir::iree_compiler::PluginRegistrar *registrar) {
  registrar->registerPlugin<mlir::iree_compiler::IREE::HAL::SA16Session>(
      "hal_target_sa16");
  return true;
}
