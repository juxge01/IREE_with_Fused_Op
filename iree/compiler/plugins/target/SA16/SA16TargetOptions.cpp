...
#include "compiler/plugins/target/SA16/SA16ResolveCPUAndCPUFeatures.h"
...
std::optional<SA16Target>
SA16Target::create(std::string_view triple, std::string_view cpu,
...
std::optional<SA16Target> SA16Target::createForHost() {
  ResolveCPUAndCPUFeaturesStatus status;
  auto triple = llvm::sys::getProcessTriple();
  auto target = SA16Target::create(triple, /*cpu=*/"host",
                                   /*cpuFeatures=*/"host",
...
void SA16Target::print(llvm::raw_ostream &os) const {
  os << "SA16Target{\n"
...
void SA16Target::storeToConfigAttrs(MLIRContext *context,
  ...
  if (sanitizerKind != DEFAULT_SANITIZER_KIND) {
    // TODO: executable format should be a plugin-level option.
  ...
  addString("executable_format", "accel-bytecode-fb");
}

std::optional<SA16Target>
SA16Target::loadFromConfigAttr(Location loc, DictionaryAttr config,
                               const SA16Target &defaultTarget) {
  ...
  if (triple || cpu || cpuFeatures) {
    ...
    ResolveCPUAndCPUFeaturesStatus status;
    std::optional<SA16Target> maybeTarget =
        SA16Target::create(*triple, cpu.value_or(""), cpuFeatures.value_or(""),
                           linkEmbedded, status);
    if (status != ResolveCPUAndCPUFeaturesStatus::OK) {
...
void SA16Target::populateDefaultsFromTargetMachine() {
...
createTargetMachine(const SA16Target &target) {
...
//
// Flags *must* be added to the SA16Target if they are target-specific and
// SA16TargetOptions if they are apply to the whole backend.

void SA16TargetCLOptions::bindOptions(OptionsBinder &binder) {
  // Initialize LLVM targets prior to the iree-sa16-list-targets CLI opt.
  initializeLLVMTargets();

  static llvm::cl::OptionCategory category("SA16 HAL Target");

  // General flags.
  binder.opt<bool>(
      "iree-sa16-list-targets", listTargets, llvm::cl::cat(category),
  ...
  binder.opt<bool>(
      "iree-sa16-debug-symbols", debugSymbols, llvm::cl::cat(category),
      llvm::cl::desc("Generate and embed debug information (DWARF, PDB, etc)"));
  binder.opt<bool>("iree-sa16-loop-interleaving", llvmLoopInterleaving,
                   llvm::cl::cat(category),
                   llvm::cl::desc("Enable LLVM loop interleaving opt"));
  binder.opt<bool>("iree-sa16-loop-vectorization", llvmLoopVectorization,
                   llvm::cl::cat(category),
                   llvm::cl::desc("Enable LLVM loop vectorization opt"));
  binder.opt<bool>("iree-sa16-loop-unrolling", llvmLoopUnrolling,
                   llvm::cl::cat(category),
                   llvm::cl::desc("Enable LLVM loop unrolling opt"));
  binder.opt<bool>("iree-sa16-slp-vectorization", llvmSLPVectorization,
                   llvm::cl::cat(category),
                   llvm::cl::desc("Enable LLVM SLP Vectorization opt"));
  binder.opt<SanitizerKind>(
      "iree-sa16-sanitize", sanitizerKind, llvm::cl::cat(category),
  ...
  binder.opt<std::string>(
      "iree-sa16-target-abi", targetABI, llvm::cl::cat(category),
      llvm::cl::desc("LLVM target machine ABI; specify for -mabi"));
  binder.opt<llvm::FloatABI::ABIType>(
      "iree-sa16-target-float-abi", targetFloatABI, llvm::cl::cat(category),
  ...
  binder.opt<std::string>(
      "iree-sa16-target-data-layout", targetDataLayout,
      llvm::cl::cat(category),
      llvm::cl::desc("LLVM target machine data layout override."));
  binder.opt<unsigned>("iree-sa16-target-vector-width-in-bytes",
  ...
  binder.opt<llvm::cl::PowerOf2ByteSize>(
      "iree-sa16-stack-allocation-limit", targetMaxStackAllocSizeInBytes,
      llvm::cl::cat(category),
      llvm::cl::desc(
          "Maximum allowed stack allocation size for LLVM CPU in bytes"));
  binder.opt<std::string>(
      "iree-sa16-enable-ukernels", enableUkernels, llvm::cl::cat(category),
  ...
  binder.opt<bool>(
      "iree-sa16-link-ukernel-bitcode", linkUKernelBitcode,
      llvm::cl::cat(category),
      llvm::cl::desc(
          "Link ukernel bitcode libraries into generated executables"));
}

SA16TargetOptions SA16TargetCLOptions::getTargetOptions() {
  ...
  ResolveCPUAndCPUFeaturesStatus status;
  std::optional<SA16Target> maybeTarget = SA16Target::create(
  ...
  SA16Target &target = targetOptions.target;
