...
#ifndef IREE_COMPILER_PLUGINS_TARGET_SA16_SA16LINKERTOOL_H_
#define IREE_COMPILER_PLUGINS_TARGET_SA16_SA16LINKERTOOL_H_
...
inline Artifact Artifact::fromFile(StringRef path) {
  return {path.str(), nullptr};
}

inline Artifact Artifact::createTemporary(StringRef prefix, StringRef suffix) {
...
inline Artifact Artifact::createVariant(StringRef basePath, StringRef suffix) {
...
inline void Artifact::keep() const {
  if (outputFile)
    outputFile->keep();
}

inline std::optional<std::vector<int8_t>> Artifact::read() const {
...
inline void Artifact::close() { outputFile->os().close(); }

inline void Artifacts::keepAllFiles() {
  libraryFile.keep();
  debugFile.keep();
...
#endif // IREE_COMPILER_PLUGINS_TARGET_SA16_SA16LINKERTOOL_H_
