...
#define GEN_PASS_DEF_ACCELCHECKIRBEFORELLVMCONVERSIONPASS
#include "iree/compiler/Codegen/ACCEL/Passes.h.inc"

static llvm::cl::opt<unsigned> clAssumedVscaleValue(
    "iree-accel-stack-allocation-assumed-vscale",
    llvm::cl::desc(
        "assumed value of vscale when checking (scalable) stack allocations"),
    llvm::cl::init(1));

namespace {
struct ACCELCheckIRBeforeLLVMConversionPass
    : impl::ACCELCheckIRBeforeLLVMConversionPassBase<
          ACCELCheckIRBeforeLLVMConversionPass> {
  using impl::ACCELCheckIRBeforeLLVMConversionPassBase<
      ACCELCheckIRBeforeLLVMConversionPass>::
      ACCELCheckIRBeforeLLVMConversionPassBase;
  void runOnOperation() override;
};
} // namespace

/// Returns success if the cumulative stack allocation size is less than the
/// limit set through --iree-accel-stack-allocation-limit (or the default
/// defined for HAL ACCEL target).
...
void ACCELCheckIRBeforeLLVMConversionPass::runOnOperation() {
