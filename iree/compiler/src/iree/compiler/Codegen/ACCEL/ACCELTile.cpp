...
#define DEBUG_TYPE "iree-accel-tile"

namespace mlir::iree_compiler {

#define GEN_PASS_DEF_ACCELTILEPASS
...
/// lowering_config.
struct ACCELTilePass : impl::ACCELTilePassBase<ACCELTilePass> {
  using impl::ACCELTilePassBase<ACCELTilePass>::ACCELTilePassBase;
...
void ACCELTilePass::runOnOperation() {
  if (tilingLevel == -1) {
...
createACCELTilePass(int64_t tilingLevel, bool skipRootOp) {
  ACCELTilePassOptions options;
  options.tilingLevel = tilingLevel;
  options.skipRootOp = skipRootOp;
  return std::make_unique<ACCELTilePass>(options);
}
