...
rewriteFlowDispatchRegionToFlowDispatchWorkgroups(
  ...
  // Propagate any non-dialect attributes (e.g., offload metadata) to ensure
  // downstream passes can still observe them.
  for (const NamedAttribute &attr : regionOp->getAttrs()) {
    workgroupsOp->setAttr(attr.getName(), attr.getValue());
  }
