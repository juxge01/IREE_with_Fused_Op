      ...
      .Default([&](Operation *op) -> SmallVector<Range> {
        if (op->hasTrait<OpTrait::Elementwise>() && op->getNumResults() == 1) {
          if (isa<RankedTensorType>(op->getResult(0).getType())) {
            return getLoopRangesFromValue(op->getResult(0), loc, builder);
          }
        }
        llvm_unreachable("op not supported for getLoopRanges");
        return {};
      });
}
...
SmallVector<Value> getWorkloadForRootOp(OpBuilder &builder, Operation *rootOp) {
...
FailureOr<IREE::Flow::DispatchRegionOp>
wrapOpsInDispatchRegion(RewriterBase &rewriter, ArrayRef<Operation *> ops) {
  if (ops.empty()) {
    return failure();
  }

  // 1. Find results that escape the cluster.
  llvm::SetVector<Value> resultsSet;
  for (Operation *op : ops) {
    for (Value result : op->getResults()) {
      for (OpOperand &use : result.getUses()) {
        Operation *user = use.getOwner();
        if (llvm::find(ops, user) == ops.end()) {
  ...
  SmallVector<Value> results(resultsSet.begin(), resultsSet.end());

  // 2. Create the dispatch region op.
  OpBuilder::InsertionGuard g(rewriter);
  Operation *firstOp = ops.front();
  rewriter.setInsertionPoint(firstOp);
  Location loc = firstOp->getLoc();

  // Calculate workload from the last op in the cluster (heuristic).
  Operation *rootOp = ops.back();
  SmallVector<Value> workload = getWorkloadForRootOp(rewriter, rootOp);

  // Calculate dynamic dimensions for results.
  SmallVector<Value> resultDynamicDims;
  for (Value res : results) {
    if (failed(reifyDynamicResultDims(rewriter, res, resultDynamicDims))) {
      return firstOp->emitOpError("failed to reify result dynamic dims");
    }
  }

  auto regionOp = rewriter.create<IREE::Flow::DispatchRegionOp>(
      loc, ValueRange(results).getTypes(), resultDynamicDims, workload);

  // 3. Move ops into the region and create a terminator.
  ...
  // 4. Replace uses.
  rewriter.replaceUsesWithIf(results, regionOp.getResults(),
                             [&](OpOperand &use) { return !regionOp->isProperAncestor(use.getOwner()); });
  return regionOp;
}
