...
static LogicalResult convertDispatchWorkgroupsToDispatchOp(
  ...
  auto dispatchOp = builder.create<IREE::Flow::DispatchOp>(
      ...
      dispatchWorkgroupsOp.getResultDims(),
      dispatchWorkgroupsOp.getArguments(), dispatchWorkgroupsOp.getArgumentDims(),
      dispatchWorkgroupsOp.getTiedOperandsAttr());
  dispatchOp->setAttr("dispatch.id",
                      builder.getI64IntegerAttr(dispatchId));
...
static LogicalResult outlineDispatchWorkgroupsOp(
  ...
  // For debugging/analysis, preserve any attributes from the original
  // workgroups op on the executable op. This is useful for carrying over
  // metadata like total FLOP counts calculated by upstream passes.
  for (const NamedAttribute &attr : dispatchWorkgroupsOp->getAttrs()) {
    // 'offload.target' 속성을 flow.executable로 전달합니다.
    // 이 정보는 Codegen 단계에서 올바른 백엔드를 선택하는 데 사용됩니다.
    if (attr.getName() == "offload.target") {
      executableOp->setAttr(attr.getName(), attr.getValue());
      continue;
    }
    executableOp->setAttr(attr.getName(), attr.getValue());
  }
  ...
  return convertDispatchWorkgroupsToDispatchOp(
      dispatchWorkgroupsOp, executableOp, exportOp, dispatchId);
}
...
struct OutlineDispatchRegionsPass
  ...
  void runOnOperation() override {
    ...
    int dispatchIdCounter = 0;
    int funcLikeCount = 0;
    for (auto funcOp : getOperation().getOps<mlir::FunctionOpInterface>()) {
      // Do not try to outline from functions that are already in executables.
      if (funcOp->getParentOfType<IREE::Flow::ExecutableOp>()) {
        continue;
      }

      // 이 함수 내의 모든 `flow.dispatch.region`을 `flow.dispatch.workgroups`로
      // 먼저 변환합니다. 이렇게 하면 아래 로직에서 하나의 연산 타입만 처리하면
      // 되므로 단순화됩니다.
      SmallVector<IREE::Flow::DispatchRegionOp> regionOps;
      funcOp.walk([&](IREE::Flow::DispatchRegionOp op) {
        regionOps.push_back(op);
      });
      for (auto regionOp : llvm::reverse(regionOps)) {
        PatternRewriter rewriter(regionOp.getContext());
        if (failed(rewriteFlowDispatchRegionToFlowDispatchWorkgroups(
                regionOp, rewriter))) {
            ...
            .Case<IREE::Flow::DispatchWorkgroupsOp>(
                [&](auto dispatchWorkgroupsOp) {
                  ...
                  if (failed(outlineDispatchWorkgroupsOp(
                          dispatchName, dispatchWorkgroupsOp, dispatchIdCounter++))) {
